...
- 默认带 `session_id`，用于和 trace 侧会话字段对齐；默认不带 `session_key` / `run_id`。
...
`resourceAttributes` 会合并到 trace resource 和 metrics resource。安装脚本的 `--tag KEY=VALUE` 也会写入 `resourceAttributes`。`host` 默认自动采集，也可通过 `resourceAttributes.host` 覆盖。
