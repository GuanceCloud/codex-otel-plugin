...
- 一个 `llm` span 下可能有多个 `tool:<name>` span。
...
| 字段 | 含义 | 常见 span |
| --- | --- | --- |
| `provider_name` | 模型供应商，例如 `openai` | `agent_run`、`llm`、`tool:*` |
| `model_name` | 模型名称，例如 `gpt-5.5` | `agent_run`、`llm`、`tool:*` |
...
| `output_preview` | 输出预览，已按最大长度裁剪 | `agent_run`、`llm` |
| `output_length` | 输出预览长度 | `agent_run`、`llm` |
| `output_kind` | 输出类型，例如 `text`、`tool_call` | `llm` |
