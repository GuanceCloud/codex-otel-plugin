...
export class FileStore {
  constructor(dataDir = "data") {
    ...
    this.metricsFile = path.join(dataDir, "metrics.ndjson");
  }

  async saveBatch({ rawRequest, spans = [], metrics = [], ingest }) {
    ...
    if (metrics.length > 0) {
      ...
      await fs.appendFile(this.metricsFile, `${lines}\n`, "utf-8");
    }

    return { id, batchFile, spanCount: spans.length, metricCount: metrics.length };
  }
