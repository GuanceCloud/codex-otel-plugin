...
配置中的 `resourceAttributes` 会合并到每个 trace resource 上，适合放跨 trace/metric 统一筛选的全局 tag，例如 `deployment.environment`、`app_id`、`app_name`、`agent_type`、`agent_source`。`host` 默认自动采集，也可通过 `resourceAttributes.host` 显式覆盖。
...
| `session_create_at` | 会话创建时间，优先取 `session_meta.timestamp`，否则退回 `session_meta` 行时间 | `agent_run` |
| `session_updated_at` | 当前 turn 对应的会话更新时间，当前取 `agent_run` 结束时间 | `agent_run` |
| `session_channel` | 会话来源通道，当前优先取 `session_meta.source`，再退回 `originator` / `thread_source` | `agent_run` |
