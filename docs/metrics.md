...
| 指标名 | 类型 | 单位 | 来源 | 描述 |
| --- | --- | --- | --- | --- |
| `gen_ai.workflow.duration` | Histogram | `s` | `invoke_agent` span duration | Codex turn 端到端工作流耗时。 |
...
| tag | 来源 | 说明 |
| --- | --- | --- |
| `final_status` | `invoke_agent.attributes.final_status` | `completed`、`cancelled`、`unset` 或 `error`。 |
