...
当前 skill 维度字段同时保留兼容 `skill.*`，并补充 `gen_ai.skill.*` 项目扩展字段；`description` 和 `version` 只在本地 skill 元数据可稳定提取时生成。具体字段见 [docs/traces.md](docs/traces.md) 和 [docs/metrics.md](docs/metrics.md)。
