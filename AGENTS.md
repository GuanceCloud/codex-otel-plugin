...
`gen_ai.agent.operation.count` / `gen_ai.agent.operation.duration` 当前用于 `llm`、`skill:*`、`tool:*`；兼容维度 `operation_name` 固定映射为 `model`、`skill`、`tool`，并同时保留 `gen_ai.operation.name=chat|skill|execute_tool`。其中 `gen_ai.agent.operation.count` 按 turn 内同维度 operation 聚合后上报，`gen_ai.agent.operation.duration` 仍按 span 上报。
